...
test.describe('About Us Page Tests', () => {
  ...
  test('should display the travel image', async ({ page }) => {
    const image = page.locator('img[alt="Travel in Nepal"]');
    await expect(image).toBeVisible();
    await expect(image).toHaveAttribute('src', '/assets/images/about-us.jpg');
  });
  ...
  test('should display Our Mission section', async ({ page }) => {
    const missionHeading = page.locator('h2:text("Our Mission")');
    await expect(missionHeading).toBeVisible();
  });

  // 9. Verify mission statement text exists
  test('should display the mission statement', async ({ page }) => {
    const missionText = page.locator('p:text("Our goal is to provide travelers with authentic")');
